...
if (config.hooks["shell.openExternal"]) {
    log("shell.openExternal hook enabled!");
...
if (config.hooks.sendMessage) {
    log("sendMessage hook enabled!");
    // FIXME
    IpcInterceptor.interceptIpcSendEvents(
        "nodeIKernelMsgService/sendMsg",
        (...args) => {
            log("Intercepted sendMsg:", args);
            ...
            if (elements?.length) {
                for (const element of elements) {
                    ...
                    textEl.content = qwqnt.main.deasync.await(
                        purifyText(textEl.content),
                    );
                }
            }
